...
copyButton.style.padding = '4px 12px'; // パディング
...
copyButton.onclick = function() {
  ...
  problemNode.querySelectorAll('.katex-mathml').forEach(ele => ele.remove());

  const formatedProblemText = problemNode.innerText
    .replace(/^配点 : \d+ 点\n/, '') // 配点を削除
    .replace(/\t/g, '') // タブを削除
  ...
  const formatedConstraintsText = partsOne
  .innerText
  .replace(/制約/, '\n制約') // 制約の前の改行を残す

  const partsArray = Array.prototype.slice.call(parts);
  const sampleNodes = partsArray
  .slice(2)
  .map(part => part.cloneNode(true))
  sampleNodes.forEach(sample => sample.querySelectorAll('.katex-mathml').forEach(ele => ele.remove()));
  const samples = sampleNodes.map(part => part.innerText.replace(/。\n/g, '。')).join('');

  const formatedSamples = samples
    .replace(/入力例 (\d)/g, '\n入力例 $1\n') // 入力例の前の改行を残す
    .replace(/出力例 (\d)/g, '\n出力例 $1\n') // 出力例の前の改行を残す
  ...
  const valueText = `${formatedProblemText}\n${formatedConstraintsText}\n${formatedSamples}`.replace(/\n\n+/g, '\n\n'); 
